...
class TongueDataset(Dataset):
    def __init__(self, csv_file, body_img_dir, edge_img_dir, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        elif labels[1] == 1 or labels[2] == 1:  # TipSideRed or Spot
            color_label[3] = 1  # Red
        
        fur_label = torch.tensor([1.0 if labels[2] == 1 or labels[7] == 1 or labels[6] == 1 else 0.0])
        
        return body_image, edge_image, labels, color_label, fur_label

def get_dataloaders(train_csv, val_csv, body_img_dir, edge_img_dir, batch_size=32, num_workers=4):
    ...
    if train_csv is not None:
        train_dataset = TongueDataset(train_csv, body_img_dir, edge_img_dir, transform=train_transform)
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    else:
        train_loader = None

    val_dataset = TongueDataset(val_csv, body_img_dir, edge_img_dir, transform=val_transform)
