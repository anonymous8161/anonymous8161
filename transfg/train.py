...
from dataloader import get_dataloaders  # 新增
...
def setup(cfg):  # 修改: 参数从args改为cfg
    config = CONFIGS[cfg.model_type]

    # 修改：设置输出类别数为8，对应8个二分类舌像特征
    num_classes = 8
...
def valid(cfg, model, writer, test_loader, global_step):  # 修改: 参数从args改为cfg
    ...
    epoch_iterator = tqdm(test_loader,
                          ...
                          dynamic_ncols=True)
    loss_fct = nn.BCEWithLogitsLoss()  # 修改: 使用BCEWithLogitsLoss
    for step, batch in enumerate(epoch_iterator):
        batch = tuple(t.to(cfg.device) for t in batch)
        x, y = batch
        with torch.no_grad():
            logits = model(x)
            eval_loss = loss_fct(logits, y.float())  # 修改: 将标签转换为float
            eval_loss = eval_loss.mean()
            eval_losses.update(eval_loss.item())

            preds = (torch.sigmoid(logits) > 0.5).float()  # 修改: 使用sigmoid进行二分类预测
    ...
    accuracy = accuracy_score(all_labels, all_preds)  # 新增: 使用sklearn计算准确率
    f1 = f1_score(all_labels, all_preds, average='macro')  # 新增: 计算F1分数

    logger.info("\n")
    logger.info("Validation Results")
    logger.info(f"Global Steps: {global_step}")
    logger.info(f"Valid Loss: {eval_losses.avg:.5f}")
    logger.info(f"Valid Accuracy: {accuracy:.5f}")  # 新增
    logger.info(f"Valid F1 Score: {f1:.5f}")  # 新增
    
    writer.add_scalar("test/accuracy", scalar_value=accuracy, global_step=global_step)  # 新增
    writer.add_scalar("test/f1_score", scalar_value=f1, global_step=global_step)  # 新增
...
def set_seed(cfg):  # 修改: 参数从args改为cfg
