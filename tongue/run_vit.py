...
from dataloader_vit import get_dataloaders
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, save_dir):
    best_val_acc = 0.0
    num_features = model.vit.heads.head.out_features
    ...
    for epoch in range(num_epochs):
        ...
        if avg_feature_accuracy > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, 'vit_best_model.pth'))
...
def test_model(model, test_loader):
    ...
    df.to_csv("roc/vit_fold5.csv", index=False)
...
@hydra.main(version_base=None, config_path="config", config_name="vit_origin_test")
def main(cfg):
    ...
    if cfg.mode == "train":
        ...
        model = TongueNet(cfg.model.num_classes).to(device)
        model.load_state_dict(torch.load(os.path.join(cfg.training.save_dir, 'vit_best_model.pth')))
        ...
        with open('vit_test_results.txt', 'w') as f:
            f.write(f'Average Feature Accuracy: {avg_feature_accuracy:.4f}\n')
            f.write(f'Average F1 Score: {avg_f1_score:.4f}\n')
            f.write(f'Macro F1 Score: {macro_f1:.4f}\n')
            f.write(f'All Correct Accuracy: {all_correct_accuracy:.4f}\n')
            f.write('\nFeature Metrics:\n')
            for i, (acc, f1) in enumerate(zip(feature_accuracies, feature_f1_scores)):
                f.write(f'Feature {i+1}: Acc={acc:.4f}, F1={f1:.4f}\n')
