...
def get_dataloaders(train_csv, val_csv, img_dir, batch_size=32, num_workers=4):
    transform = transforms.Compose([
        ...
        # transforms.Normalize(mean=[0.45302, 0.36512, 0.38218], std=[0.27749, 0.23474, 0.24553]),
    ])

    if train_csv is not None:
        train_dataset = TongueDataset(train_csv, img_dir, transform=transform)
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    else:
        train_loader = None

    val_dataset = TongueDataset(val_csv, img_dir, transform=transform)
